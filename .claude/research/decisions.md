...
- **Drift fix 2026-05-24 (day-7 smoke):** `pubmed_parser==0.5.1` raises `KeyError('year')` on epub-only papers — `parse_date` in `pubmed_oa_parser` assumes every `<pub-date>` it falls back to has a `<year>` child, which isn't guaranteed for `pub-type="epub"` entries. Surfaced as salvage-failures on ~40% of the 100-paper COPD smoke. Patched via in-process monkey-patch on `pubmed_parser.pubmed_oa_parser.parse_date` that seeds `year=None` before the upstream code reads it (commit `12a303b`). Localized to `parse.py` module-load; no fork. **Cleanup path:** upstream PR or pinned-fork plan at M5. **Risk:** patch silently no-ops if upstream releases a 0.5.2+ that restructures `parse_date` — pin `pubmed_parser==0.5.1` in `pyproject.toml` and revisit at the next dep bump.
...
- `pubmed.elink_pubmed_to_pmc(pmids) -> {pmid: "PMC<n>"}` — multiple `&id=` params yield per-PMID linksets. PMIDs with no PMC counterpart are omitted from the result. **Drift fix 2026-05-24 (day-7 smoke):** originally implemented as one HTTP call, but NCBI stream-closes when ~100 ids land in a single URL. Now batched in chunks of **50 PMIDs per GET**, results merged client-side (commit `c72d669`). At 10 req/s the extra batches cost ~0.2 s for limit=100 — still inside the 30 s budget.
